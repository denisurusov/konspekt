...
// vocabulary and should carry no status at all. This is the subject of R2 in
// spec/architecture/SCHEMA-RECONCILIATION.md, so it reports rather than errors.
...
// not errors: observed usage diverges and R4 is unresolved, so the report is
// the evidence for that decision rather than a verdict on it.
...
export function loadInstance(instanceDir, opts = {}) {
  ...
  const checkEntity = (e) => {
    ...
    // Fields the schema does not define for this entity type. Informational:
    // this is the evidence for R1 (the "uniform envelope") and R5 (artifact
    // shape), not a verdict on either.
  ...
  for (const e of edges) {
    ...
    for (const [end, side] of [[e.from, "from"], [e.to, "to"]]) {
      ...
      // Domain/range per schema.ts. WARNING, not error: R4 is unresolved, so
      // these rows are the evidence for that decision, not a verdict.
